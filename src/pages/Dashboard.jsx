...
function DonutChart({ segments, size = 148, centerLabel }) {
  ...
  return (
    <svg width={size} height={size} style={{ flexShrink: 0 }}>
      <circle cx={cx} cy={cy} r={r} fill="none" stroke="#e8ddd0" strokeWidth={thickness} />
      {segments.map((seg, i) => {
      ...
      {centerLabel && (
        <text x={cx} y={cy - 7} textAnchor="middle" fill="#2C1810" fontSize="17" fontWeight="700">{centerLabel}</text>
      )}
      <text x={cx} y={cy + 13} textAnchor="middle" fill="#8B6F6F" fontSize="10">total</text>
    </svg>
  )
}
...
function LineChart({ data = PLACEHOLDER_REVENUE, color = '#D4A853' }) {
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W + padX * 2} ${H + padY * 2 + 24}`} style={{ overflow: 'visible' }}>
      ...
      {gridLines.map(p => {
        const y = padY + H - p * H
        return <g key={p}>
          <line x1={padX} x2={W + padX} y1={y} y2={y} stroke="#e8ddd0" strokeWidth="1" />
          <text x={padX - 6} y={y + 4} textAnchor="end" fill="#8B6F6F" fontSize="10">
      ...
      {MONTHS.map((m, i) => (
        <text key={m} x={scaleX(i)} y={padY + H + 20} textAnchor="middle" fill="#8B6F6F" fontSize="10">{m}</text>
      ))}
    </svg>
  )
}
...
/* ─── Styles ─── */
const S = {
  page:       { background: '#FAF7F2', minHeight: '100vh', padding: '28px 28px 48px', color: '#2C1810' },
  // Header
  headerRow:  { display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 12 },
  title:      { fontSize: 22, fontWeight: 700, color: '#2C1810', margin: '0 0 4px' },
  subtitle:   { fontSize: 13, color: '#8B6F6F', margin: 0 },
  dateBadge:  { background: '#ffffff', border: '1px solid #e8ddd0', borderRadius: 8, padding: '7px 14px', fontSize: 12, color: '#8B6F6F', whiteSpace: 'nowrap' },
  // KPI
  kpiGrid:    { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 16, marginBottom: 24 },
  kpiCard:    { background: '#ffffff', borderRadius: 14, padding: '20px 22px', border: '1px solid #e8ddd0', cursor: 'pointer', transition: 'border-color 0.2s, transform 0.15s', position: 'relative', overflow: 'hidden' },
  kpiIcon:    { fontSize: 22, marginBottom: 10 },
  kpiVal:     { fontSize: 32, fontWeight: 800, lineHeight: 1, marginBottom: 6 },
  kpiLabel:   { fontSize: 12, color: '#8B6F6F', fontWeight: 500 },
  kpiTrend:   { position: 'absolute', top: 16, right: 16, fontSize: 11, fontWeight: 600, padding: '3px 8px', borderRadius: 20 },
  // Section title
  secTitle:   { fontSize: 13, fontWeight: 700, color: '#8B6F6F', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 16 },
  // Cards
  card:       { background: '#ffffff', borderRadius: 14, border: '1px solid #e8ddd0', padding: '20px 22px' },
  cardHead:   { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 },
  cardTitle:  { fontSize: 14, fontWeight: 600, color: '#2C1810' },
  seeAll:     { fontSize: 12, color: '#D4A853', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600, padding: 0 },
  // Row items
  row:        { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', borderRadius: 8, marginBottom: 6, background: '#FAF7F2' },
  rowTitle:   { fontSize: 13, fontWeight: 600, color: '#2C1810', marginBottom: 2 },
  rowSub:     { fontSize: 11, color: '#8B6F6F' },
  rowRight:   { textAlign: 'right' },
  rowDate:    { fontSize: 12, fontWeight: 600, color: '#8B6F6F' },
  // Badges
  ...
  legendVal:  { fontWeight: 700, color: '#2C1810', marginLeft: 'auto', paddingLeft: 12 },
  // Line chart
  lineCard:   { background: '#ffffff', borderRadius: 14, border: '1px solid #e8ddd0', padding: '20px 22px', marginBottom: 24 },
  // Bottom row
  bottomRow:  { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 },
  // Quick guide
  guideItem:  { display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: 8, background: '#FAF7F2', marginBottom: 8, cursor: 'pointer', transition: 'background 0.15s' },
  ...
  guideTitle: { fontSize: 13, fontWeight: 600, color: '#2C1810', marginBottom: 2 },
  guideSub:   { fontSize: 11, color: '#8B6F6F' },
  empty:      { textAlign: 'center', padding: '28px 0', color: '#8B6F6F', fontSize: 13 },
}
...
export default function Dashboard() {
  ...
  const payBadge = {
    ...
    balance: { background: 'rgba(212,168,83,0.15)', color: '#D4A853' },
  ...
  const kpiCards = [
    { icon: '📅', label: t(lang, 'eventsAVenir'), value: stats.events,     color: '#D4A853', trend: '+2', trendUp: true,  route: '/evenements' },
    { icon: '👤', label: t(lang, 'clients'),       value: stats.clients,    color: '#6B2737', trend: '+5', trendUp: true,  route: '/evenements' },
    { icon: '💰', label: t(lang, 'revenuesMois'),  value: fmt(monthRevenue) + ' MAD', color: '#D4A853', trend: '', route: '/paiements', isText: true },
    { icon: '📦', label: t(lang, 'alertesStock'),  value: stats.stockAlerts, color: stats.stockAlerts > 0 ? '#ef4444' : '#22c55e', trend: stats.stockAlerts > 0 ? '⚠' : '✓', trendUp: stats.stockAlerts === 0, route: '/stock' },
  ]

  const eventTypeSegments = [
    { pct: 69, color: '#D4A853', label: 'Mariages',  val: '69%' },
    { pct: 20, color: '#6B2737', label: 'Corporate', val: '20%' },
    { pct: 11, color: '#8B6F6F', label: 'Privé',     val: '11%' },
  ]
  const revenueSegments = [
    { pct: 50, color: '#D4A853', label: 'Traiteur',   val: '50%' },
    { pct: 30, color: '#6B2737', label: 'Pâtisserie', val: '30%' },
    { pct: 20, color: '#8B6F6F', label: 'Décoration', val: '20%' },
  ]
  ...
  if (loading) return (
    <div style={{ ...S.page, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ color: '#8B6F6F', fontSize: 14 }}>{t(lang, 'loading')}</div>
    </div>
  )
      ...
      <div style={S.kpiGrid}>
        {kpiCards.map(c => (
          <div key={c.label} style={S.kpiCard}
            ...
            onMouseLeave={e => { e.currentTarget.style.borderColor = '#e8ddd0'; e.currentTarget.style.transform = 'none' }}
          >
            {/* glow blob */}
            ...
            {c.trend && (
              <div style={{ ...S.kpiTrend, background: c.trendUp ? 'rgba(212,168,83,0.12)' : 'rgba(239,68,68,0.12)', color: c.trendUp ? '#D4A853' : '#ef4444' }}>
                {c.trend}
              </div>
            )}
          </div>
      ...
      <div style={S.donutRow}>
        ...
        <div style={S.card}>
          <div style={S.cardHead}>
            ...
            <span style={{ ...S.badge, background: 'rgba(212,168,83,0.12)', color: '#D4A853' }}>MAD</span>
          </div>
          <div style={S.donutInner}>
      ...
      <div style={S.lineCard}>
        <div style={{ ...S.cardHead, marginBottom: 20 }}>
          ...
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 22, fontWeight: 800, color: '#D4A853' }}>{fmt(PLACEHOLDER_REVENUE.reduce((a, b) => a + b, 0))} MAD</div>
            <div style={{ fontSize: 11, color: '#6b7280' }}>cumul annuel (estimé)</div>
          </div>
        </div>
        <LineChart data={PLACEHOLDER_REVENUE} color="#D4A853" />
      </div>
            ...
            <>
              ...
              {recentPayments.slice(0, 2).map(p => (
                <div key={p.id} style={S.row}>
                  ...
                  <div style={S.rowRight}>
                    <div style={{ fontSize: 14, fontWeight: 700, color: '#D4A853' }}>+{fmt(p.amount)} MAD</div>
        ...
        <div style={S.card}>
          ...
          {[
            ...
            >
              ...
              <div style={{ marginLeft: 'auto', color: '#8B6F6F', fontSize: 14 }}>›</div>
            </div>
          ))}
        </div>
